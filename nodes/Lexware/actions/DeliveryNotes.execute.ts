...
function buildDeliveryNoteBody(
...
export async function executeDeliveryNotes(
  ...
  switch (operation) {
    case "create": {
      ...
      const deliveryNote = buildDeliveryNoteBody(getParam, i);
    ...
    case "createByJson": {
      ...
      const standard = buildDeliveryNoteBody(getParam, i);
    ...
    case "get": {
      const deliveryNoteId = this.getNodeParameter(
        "deliveryNoteId",
        i
      ) as string;
      responseData = await lexwareApiRequest.call(
        this,
        "GET",
        `/v1/delivery-notes/${deliveryNoteId}`
      );
      break;
    }
    ...
    case "pursue": {
      ...
      responseData = await lexwareApiRequest.call(
        this,
        "POST",
        `/v1/delivery-notes/${deliveryNoteId}/pursue`,
        body
      );
      break;
    }
    ...
    case "renderPdf": {
      ...
      responseData = await lexwareApiRequest.call(
        this,
        "GET",
        `/v1/delivery-notes/${deliveryNoteId}/document`
      );
      break;
    }
    ...
    case "downloadFile": {
      ...
      const response = await lexwareApiDownload.call(
        this,
        `/v1/delivery-notes/${deliveryNoteId}/files/${fileId}`
      );

      const binaryData = await this.helpers.prepareBinaryData(
        response.body as Buffer,
        `delivery-note-${deliveryNoteId}.pdf`,
        "application/pdf"
      );
